...
def lambda_handler(event, context):
    try:
        ...
        # Query DynamoDB for items associated with the user ID
        response = table.scan(
        ...
        if 'Items' not in response or not response['Items']:
            return {'statusCode': 404, 'body': json.dumps('No active items found for user')}

        active_items = [item for item in response['Items'] if not item.get('is_expired', True)]
        return {'statusCode': 200, 'body': json.dumps(active_items)}
